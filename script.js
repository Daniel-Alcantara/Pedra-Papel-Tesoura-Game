...
$(document).ready(function () {
    ...
    $(document).on("click", ".papel_item", function () {

        var player_select = "papel"

        var box_add = $(this)

        $(".selecionar_item").css("display", "none")
        $(".box_item_selecionado").css("display", "flex")

        $(".box_item_selecionado .item_box_player").append(box_add)

        bot_select = Bot_select_item();

        $(".pedra_item").css("margin", "unset")
        $(".papel_item").css("margin", "unset")
        $(".tesoura_item").css("margin", "unset")

        Calcular_resultado(player_select, bot_select);

    })

    $(document).on("click", ".pedra_item", function () {

        var player_select = "pedra"
    ...
    $(document).on("click", ".tesoura_item", function () {

        var player_select = "tesoura"
    ...
    $(document).on("click", ".jogar_novamente", function(){

        $(".selecionar_item").css("display", "block")
        $(".box_item_selecionado").css("display", "none")

        $(".item_box_player").html("")
        $(".item_box_bot").html("")

        $(".selecionar_item").html("")
        $(".selecionar_item").html(`
        <div class="item_box">
        <div class="papel_item box_item">
        ...
        <div class="tesoura_item box_item">
...
function Bot_select_item() {
    ...
    bot_select = ArrayItens[(Math.random() * ArrayItens.length) | 0]

    if (bot_select == "pedra") {
        console.log("bot pedra")
        $(".box_item_selecionado .item_box_bot").append(`
                <div class="pedra_item box_item">
                    <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48">
                    <path fill="#3B4262"
                    d="M45.06 12.22c-.642-8.096-9.734-7.269-9.734-7.269-3.837-6.765-9.832-1.865-9.832-1.865-4.606-6.63-10.38-.486-10.38-.486-9.957-1.074-9.571 7.066-9.571 7.066-.234 2.588 1.403 10.593 1.403 10.593-1.477-4.614-4.68-.784-4.68-.784-3.94 6.078-.975 9.405-.975 9.405 5.33 6.246 16.688 13.743 16.688 13.743 4.113 2.356 2.373 4.457 2.373 4.457l24.876-4.11.571-4.718c3.782-11.436-.739-26.032-.739-26.032z" />
    ...
    else if (bot_select == "papel") {
        console.log("bot papel")
    ...
    else if (bot_select == "tesoura") {
        console.log("bot tesoura")
...
// Resultado do Jogo
function Calcular_resultado(playerSelecionou, botSelecionou) {

    if(playerSelecionou == "papel" && botSelecionou == "pedra"){
        $(".mensagem_resultado p").text("Você Ganhou")
        score = score + 1
        $("#pontuacao_user").html(score)
    }
    if(playerSelecionou == "papel" && botSelecionou == "papel"){
        $(".mensagem_resultado p").text("Você Empatou")
    }
    if(playerSelecionou == "papel" && botSelecionou == "tesoura"){
        $(".mensagem_resultado p").text("Você Perdeu")
    }

    if(playerSelecionou == "pedra" && botSelecionou == "pedra"){
        $(".mensagem_resultado p").text("Você Empatou")
    }
    if(playerSelecionou == "pedra" && botSelecionou == "tesoura"){
        $(".mensagem_resultado p").text("Você Ganhou")
        score = score + 1
        $("#pontuacao_user").html(score)
    }
    if(playerSelecionou == "pedra" && botSelecionou == "papel"){
        $(".mensagem_resultado p").text("Você Perdeu")
    }

    if(playerSelecionou == "tesoura" && botSelecionou == "tesoura"){
        $(".mensagem_resultado p").text("Você Empatou")
    }
    if(playerSelecionou == "tesoura" && botSelecionou == "pedra"){
        $(".mensagem_resultado p").text("Você Perdeu")
    }
    if(playerSelecionou == "tesoura" && botSelecionou == "papel"){
        $(".mensagem_resultado p").text("Você Ganhou")
        score = score + 1
        $("#pontuacao_user").html(score)
    }
